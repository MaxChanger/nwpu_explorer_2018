...
*/
      ...
      ROS_WARN("curr_x %f,curr_y %f,distance: %f",coords.x(),coords.y(),diff);
	    if ( diff > 1 /*&& last_time + ros::Duration(time_torlerence_) > ros::Time::now()*/ ) 
      { 
          ...
          bool flag=true;
          for(hector_worldmodel_msgs::ObjectModel::_objects_type::const_iterator tempIt=it;
          tempIt!=data.response.model.objects.end();++tempIt)
          {

            ROS_ERROR("in for");
            const hector_worldmodel_msgs::Object& compDis = *tempIt;
            Eigen::Vector2f coords_temp;
            coords_temp.x() = compDis.pose.pose.position.x;
            coords_temp.y() = compDis.pose.pose.position.y;
            double dis = 0;
            dis = (coords_temp.x() - coords.x()) * (coords_temp.x() - coords.x()) + (coords_temp.y() - coords.y()) * (coords_temp.y() - coords.y()); 
            if(dis<1&&dis!=0)
            flag=false;
          }
          if(!flag)
