...
public:
  ...
  {
    ROS_WARN("This version is The April");
    ros::NodeHandle nh;
    ...
    exploration_plan_generation_timer_ = nh.createTimer(ros::Duration(5.0), &SimpleExplorationController::timerPlanExploration, this, false );
  ...
  void timerCmdVelGeneration(const ros::TimerEvent& e)
  {
    ...
    if( fabs(twist.linear.x) < 0.1 && fabs(twist.linear.y) < 0.1 && fabs(twist.angular.z) < 0.3 )
    {	
    ...
    if (fabs(twist.angular.z) > M_PI)
    {
     ...
     twist.angular.z *= vel_factor * 1.4 ;//1.0调整大小
  ...
  void timerCmd_Sun(const ros::TimerEvent& e)
  {
    ...
    ROS_ERROR("[timerCmd_Sun] vel pub %lf %lf ==>%lf", twist.linear.x, twist.linear.y ,twist.angular.z);
    vel_pub_.publish(twist);//发送给底盘
    sleep(1);
  }
