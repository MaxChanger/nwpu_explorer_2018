...
ExplorerTeleop::ExplorerTeleop():
    ...
    linear_turn_left_right(joy_msg->left_right),
    // 用于重新开启底盘
    ...
    ph_.param("l_scale_init",   l_scale,        1.2);//0.3 0.6 1.6
    ph_.param("l_scale_full",   l_scale_full,   1.8);
    ph_.param("l_scale_mid",    l_scale_mid,    1.2);
    ph_.param("l_scale_slow",   l_scale_slow,   0.8);

    ph_.param("a_scale_init",   a_scale,        1.0);// 0.1 0.4 0.6
    ph_.param("a_scale_full",   a_scale_full,   1.5);
    ph_.param("a_scale_mid",    a_scale_mid,    1.0);
    ph_.param("a_scale_slow",   a_scale_slow,   0.5);
...
void ExplorerTeleop::joyCallback(const sensor_msgs::Joy::ConstPtr &joy) {
    ...
    if(joy_msg->askForButton(basic_mode_button)){
        if(joy_msg->askForButton(vice_wheel_mode_button)){
            if(std::fabs(joy_msg->askForAxes(left_back_vice_wheel_up_down) )> 10e-6) {//后副履带单独移动
            vice_wheel_publisher.back_left_wheel_angular = -(joy_msg->askForAxes(left_back_vice_wheel_up_down)) * vice_whell_;
            
        }
            if(std::fabs(joy_msg->askForAxes(right_back_vice_wheel_up_down) )> 10e-6) {
            ...
            if(joy_msg->askForButton(relive)){
            ...
            if(std::fabs(joy_msg->askForAxes(right_front_vice_wheel_up_down)) > 10e-6) {
            vice_wheel_publisher.front_right_wheel_angular = -(joy_msg->askForAxes(right_front_vice_wheel_up_down)) * vice_whell_;
            }
            
            if (std::fabs(joy_msg->askForAxes(linear_turn_left_right)) > 10e-6) {
                vel_publisher.linear.y= joy_msg->askForAxes(linear_turn_left_right) * l_scale *0.5 ;
            }
            if (std::fabs(joy_msg->askForAxes(linear_front_back)) > 10e-6) {
                vel_publisher.linear.x = joy_msg->askForAxes(linear_front_back) * l_scale *0.5;
            }
            if (std::fabs(joy_msg->askForAxes(angular_left_right)) > 10e-6) {
                vel_publisher.angular.z = joy_msg->askForAxes(angular_left_right) * a_scale *1.2;
            }
        }
    }
    ...
    if (joy_msg->askForButton(arm_control_button)) {
        if (joy_msg->askForButton(arm_driect_control_button)) {
            if (std::fabs(joy_msg->askForAxes(arm_control_forward_back)) > 10e-6) {
                arm_driect_publisher.twist.linear.z = -arm_scale * joy_msg->askForAxes(arm_control_forward_back);//小臂上下
            }

            if (std::fabs(joy_msg->askForAxes(arm_control_left_right)) > 10e-6) {
                arm_driect_publisher.twist.linear.y = arm_scale_first * joy_msg->askForAxes(arm_control_left_right);//大臂左右
            }

            if (std::fabs(joy_msg->askForAxes(arm_control_up_down)) > 10e-6) {
                arm_driect_publisher.twist.linear.x = arm_scale_first * joy_msg->askForAxes(arm_control_up_down);//整体上下
            }

            if (std::fabs(joy_msg->askForAxes(camera_control_up_down)) > 10e-6) {
                arm_driect_publisher.twist.angular.x = -arm_camera_scale * joy_msg->askForAxes(camera_control_up_down);//摆动
            }
        ...
        else {//需要具体在rvi中根据方位在测试 现阶段 button前正、左旋为正  
            ...
            if(std::fabs(joy_msg->askForAxes(arm_moveit_forward)) > 10e-6){ //左轴上下 整体前进后退
            arm_moveit_publisher.x = arm_scale_moveit * (joy_msg->askForAxes(arm_moveit_forward));
            }
            ...
            if(std::fabs(joy_msg ->askForAxes(arm_paw_rotateleft)) > 10e-6){ //右轴左右 左右旋转
            arm_moveit_publisher.rotate = -arm_scale_moveit1 * (joy_msg ->askForAxes(arm_paw_rotateleft) );
        }
            if(std::fabs(joy_msg->askForAxes(paws_control_open_close)) > 10e-6){ //2 4 爪子开合
                paw_move_msg.data = joy_msg->askForAxes(paws_control_open_close) * arm_camera_scale;
            }
        }
...
void ExplorerTeleop::messageClean() {
    ...
    this->arm_driect_publisher = arm_empty1;
    std_msgs::Float32 float_empty;
